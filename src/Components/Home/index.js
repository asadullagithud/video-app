...
class Home extends Component {
  ...
  render (){
    const {activeID, searchVal} = this.state
    const searchResults = videoLists.filter(eachUser =>
      eachUser.title.includes(searchVal)
    )
  return (
    <div className="container">
      <div className="videoPlayer">
        <ReactPlayer url={activeID} controls width={950} height={550} />
      </div>
      <div className="container01">
        ...
        <ul className="videoList">
          {searchResults.map(each => (
            <VideoList key={each.id} videoDetails={each} isActive={activeID === each.URL} onclickPlayId={this.onclickPlayId} />
          ))}
        </ul>
      </div>
    </div>
